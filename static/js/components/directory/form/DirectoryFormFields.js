  ...
  render(containerElement) {
    ...
    fieldsBlock.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:6px; background:#f1f5f9; padding:10px; border-radius:6px; border:1px solid #e2e8f0;">
        ...
        <select id="form-parent-node-select" style="padding:6px; font-size:0.9rem; border-radius:4px; border:1px solid #cbd5e1; cursor:pointer;"></select>
      </div>
    ...
    this.form.ctx.entity.columns.forEach(colName => {
      ...
      if (type === 'TEXT' && colName.toLowerCase().includes('описание')) {
        ...
        textarea.style.cssText = 'width:100%; height:100px; padding:8px; border:1px solid #cbd5e1; border-radius:4px; font-family:inherit; font-size:0.9rem; resize:vertical;';
